    ...
    if (loginForm) {
        ...
        loginForm.addEventListener('submit', (e) => {
            ...
            // Lógica específica para o Antônio
                ...
                if (passwordInput.value !== 'amo3ceiroB') {
    ...
    if (tabBtns.length > 0) {
        ...
        tabBtns.forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                // Atualiza o cardápio se for para a aba menu
                if (targetTab === 'menu') renderMenu();

                // Atualiza as reservas se for para a aba reservas
        ...
        const renderMenu = () => {
            ...
            days.forEach((dayName, index) => {
                ...
                const mealText = menu[dayNum] || "Não informado";
                
                card.innerHTML = `
                    <span class="day">${dayName}${isToday ? ' (Hoje)' : ''}</span>
                    <p class="meal">${mealText}</p>
                `;
        ...
        if (editProfileBtn && editModal) {
            ...
            if (savedProfile) {
                ...
                // Lógica de visibilidade para a aba de Reservas (Delia, Fabiana e Antônio)
                const allowedUsers = ['Delia', 'Fabiana', 'Antônio'];
                if (savedProfile && allowedUsers.includes(savedProfile.name)) {
                    document.querySelectorAll('.reservation-field').forEach(f => f.style.display = 'flex');
                    initReservations();
                }
            }
        }
        ...
        function initReservations() {
            const userProfile = JSON.parse(localStorage.getItem('userProfile'));
            const isFabiana = userProfile && userProfile.name === 'Fabiana';
            const storageKey = 'reservations_data_v3'; // Nova versão para suportar dias
            ...
            const shifts = {
                morning: {
                    container: document.getElementById('morningReservationsTable'),
                    pane: document.getElementById('morningResPane'),
                    times: [
                        '1ª Aula (07:35 às 08:20)', 
                        '2ª Aula (08:20 às 09:10)', 
                        '3ª Aula (09:10 às 10:00)', 
                        'Intervalo (10:00 às 10:15)', 
                        '4ª Aula (10:15 às 11:00)', 
                        '5ª Aula (11:00 às 11:45)', 
                        '6ª Aula (11:45 às 12:30)'
                    ]
                },
                afternoon: {
                    container: document.getElementById('afternoonReservationsTable'),
                    pane: document.getElementById('afternoonResPane'),
                    times: [
                        '1ª Aula (13:00 às 13:45)', 
                        '2ª Aula (13:45 às 14:35)', 
                        '3ª Aula (14:35 às 15:25)', 
                        'Intervalo (15:25 às 15:40)', 
                        '4ª Aula (15:40 às 16:25)', 
                        '5ª Aula (16:25 às 17:10)', 
                        '6ª Aula (17:10 às 17:55)'
                    ]
                },
                night: {
                    container: document.getElementById('nightReservationsTable'),
                    pane: document.getElementById('nightResPane'),
                    times: [
                        '1ª Aula (16:50 às 17:35)', 
                        '2ª Aula (17:35 às 18:25)', 
                        '3ª Aula (18:25 às 19:15)', 
                        'Intervalo (19:15 às 19:30)', 
                        '4ª Aula (19:30 às 20:15)', 
                        '5ª Aula (20:15 às 21:00)', 
                        '6ª Aula (21:00 às 21:45)'
                    ]
                }
            };
            ...
            const renderSlots = () => {
                if (!reservations[currentDay]) reservations[currentDay] = { morning: {}, afternoon: {}, night: {} };

                Object.keys(shifts).forEach(shiftKey => {
                    const shift = shifts[shiftKey];
                    if (!shift.container) return;
                    
                    shift.container.innerHTML = '';
                    shift.times.forEach((time, index) => {
                        const slotId = `${shiftKey}_${index}`;
                        const isInterval = time.includes('Intervalo');
                        
                        const dayData = reservations[currentDay][shiftKey] || {};
                        const libInfo = isInterval ? 'Intervalo' : ((dayData[slotId] && dayData[slotId].library) || 'Disponível');
                        const labInfo = isInterval ? 'Intervalo' : ((dayData[slotId] && dayData[slotId].lab) || 'Disponível');

                        const tr = document.createElement('tr');
                        tr.innerHTML = `
                            <td class="res-time-cell">${time}</td>
                            <td class="res-resource-cell ${isInterval ? 'interval-cell' : (libInfo === 'Disponível' ? 'available' : 'booked')} ${isFabiana && !isInterval ? 'editable' : ''}" data-resource="library">
                                ${libInfo}
                                ${isFabiana && !isInterval ? '<i class="fas fa-edit edit-icon-mini"></i>' : ''}
                            </td>
                            <td class="res-resource-cell ${isInterval ? 'interval-cell' : (labInfo === 'Disponível' ? 'available' : 'booked')} ${isFabiana && !isInterval ? 'editable' : ''}" data-resource="lab">
                                ${labInfo}
                                ${isFabiana && !isInterval ? '<i class="fas fa-edit edit-icon-mini"></i>' : ''}
                            </td>
                        `;

                        if (isFabiana && !isInterval) {
                            tr.querySelectorAll('.editable').forEach(cell => {
                                cell.addEventListener('click', () => {
                                    const resource = cell.dataset.resource;
                                    const resourceName = resource === 'library' ? 'Biblioteca' : 'Informática';
                                    const dayBtn = document.querySelector(`.day-nav-btn[data-res-day="${currentDay}"]`);
                                    const dayName = dayBtn ? dayBtn.textContent : "Dia";
                                    
                                    showCustomPrompt('Editar Reserva', `Defina o uso para ${resourceName} na ${dayName} às ${time.split('(')[0].trim()}:`, (value) => {
                                        if (!reservations[currentDay][shiftKey]) reservations[currentDay][shiftKey] = {};
                                        if (!reservations[currentDay][shiftKey][slotId]) reservations[currentDay][shiftKey][slotId] = {};
                                        reservations[currentDay][shiftKey][slotId][resource] = value;
                                        saveReservations();
                                        renderSlots();
                                        showToast('Reserva Atualizada', 'O horário foi atualizado com sucesso!', 'success');
                                    });
                                });
                            });
                        }

                        shift.container.appendChild(tr);
                    });
                });
            };

            // Lógica de Alternância de Abas de Turno e Dias (apenas uma vez)
            if (!reservationsInitialized) {
                // Alternar Dias
                const dayBtns = document.querySelectorAll('.day-nav-btn');
                dayBtns.forEach(btn => {
                    btn.addEventListener('click', () => {
                        currentDay = btn.dataset.resDay;
                ...
                // Alternar Turnos
                const shiftBtns = document.querySelectorAll('.shift-nav-btn');
                if (shiftBtns.length > 0) {
                    shiftBtns.forEach(btn => {
                        ...
                        newBtn.addEventListener('click', () => {
                            currentShift = newBtn.dataset.resShift;
                            document.querySelectorAll('.shift-nav-btn').forEach(b => b.classList.remove('active'));
                            newBtn.classList.add('active');
                            
                            Object.keys(shifts).forEach(key => {
                                if (shifts[key].pane) {
                                    shifts[key].pane.classList.toggle('active', key === currentShift);
                                }
                            });
                        });
                    });
